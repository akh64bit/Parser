 ...
 */
  ...
  public static SdoGeom getSdoGeomValue (int position, byte []data, int length)
		    throws java.io.IOException
		    {
		      byte tmp[] = new byte[length];  
 ...
 public static void setSdoGeomValue (SdoGeom sdoGeom, int position, byte []data)
	        throws java.io.IOException
	 {
	  /* creates a new data output stream to write data to
	   * underlying output stream
	   */
	   ...
	   outstr.write(sdoGeom.getByteArray());
	   // creates a byte array with this output stream size and the 
	   // valid contents of the buffer have been copied into it
	   byte []B = ((ByteArrayOutputStream) out).toByteArray();
